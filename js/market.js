...
const Market = (() => {
   ...
   * the chart advances. Mirrors the `bucket` candles() uses (including MAX's
   * per-asset dynamic bucket), so the UI redraws only when a new bar forms
   * (1S→every second, 1W→every week…) instead of every render tick.
   */
